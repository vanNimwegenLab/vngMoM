...
#TODO:
#We are getting rowsums from the raw pixel intensities. That should not be necessary.
#Also, it requires knowing the width of the pictures, which this script hardcodes to 20.
...
$numargs = @ARGV;
if($numargs != 1){
    die "USAGE: get_size_and_fluo_mul_fluo_channels.pl infile.\n";
}

$infile = shift(@ARGV);

#Get name of growth lane from the file
if($infile =~ /pos(\d+)/ || $infile =~ /Pos(\d+)/){
    $pos = $1;
}
else{
    $pos = "NA";
    print STDERR "cannot get position from $infile\n";
}
if($infile =~ /GL(\d+)/){
    $GL = $1;
}
else{
    print STDERR "cannot get growth lane id GL from $infile.\n";
    $GL = "NA";
}
...
if($infile =~ /ExportedCellStats\_(\d+)\_/){
    ...
    $date = "NA";
    print STDERR "cannot get date from $infile.\n";
}
...
if($infile =~ /^([\S\/\d\_]*)ExportedCellStats/){
    ...
    $prefix = "NA";
    print STDERR "cannot get prefix from $infile\n";
}
...
while(<F>){
	...
	if($curid >= 0){
	    #add stats of previous time point
	    $stats = $stats . $touching;
	    push @curstats, $stats;
	    $stats = "";
	...
	$vals =~ s/\;/ /g;
	my @in = split(/\s+/,$vals);
	#Get the row sums. We are relying on each row being 20 pixels wide
	$size = @in/20;
	my @rowsums = ();
	...
	if($isbottom){
	    ...
	    #TODO:
	    #If Florian's box does NOT extend to bottom of the growth lane ->
	    #Use standard max deriv
	    #Else if the box goes to bottom growth lane
	    #Do we see decrease? -> Touching bottom, take bottom fo growth lane
	    #If not -> Use standard max deriv.

	    
	    $crit1 = 0;
	    $crit2 = 0;
	    #First criterium: Is the endpoint above the average value in the middle? (not used right now)
